-- When social_media_plans.done changes from false to true, use a dedicated notification title/body.
-- Trigger already notifies on done change; this only makes the message user-friendly for "done = true".
...
BEGIN
  ...
  -- Dedicated notification when done goes from false to true
  IF v_done_became_true THEN
...
COMMENT ON FUNCTION public.trigger_notify_plan_status_change() IS 'Notify org members when status, production_status, or done changes on social_media_plans. Uses title "Plan selesai" when done becomes true.';
