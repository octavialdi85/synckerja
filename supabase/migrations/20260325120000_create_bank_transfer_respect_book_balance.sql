-- Ensure create_bank_transfer uses book balance from bank_account_balances only (per org),
-- never overwrites stored balance with full ledger aggregate before transfer.
...
COMMENT ON FUNCTION public.create_bank_transfer IS 'Atomic inter-bank transfer: book balances from bank_account_balances; no ledger overwrite before debit/credit';
