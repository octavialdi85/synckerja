-- Before transfer checks, align bank_account_balances with ledger (income − expense) when the row
-- drifted (e.g. bad backfill). Keeps UI and RPC consistent with transaction totals.
...
COMMENT ON FUNCTION public.create_bank_transfer IS 'Inter-bank transfer: reconciles book row from ledger (income−expense) when drifted, then debits/credits';
