-- Prorate: use end date from latest successful payment so remaining_days matches UI (e.g. "Renews 28 Maret 2027").
-- organization_subscriptions.subscription_end_date can be out of sync; payments.subscription_end_date is the
-- source of truth shown in Payment History and Overview (next billing / days remaining).
...
BEGIN
  ...
  -- Use end (and optionally start) from latest successful payment so prorate remaining_days
  -- matches the "Renews on ... / X days remaining" shown in the UI (Payment History / Overview).
  -- If payment has no subscription_end_date (e.g. not backfilled), derive from created_at + billing_cycle.
  -- Only consider payments that have already occurred (created_at <= now) so we use the current period end, not a future one.
  ...
  IF FOUND AND v_payment.subscription_end_date IS NOT NULL THEN
    ...
    -- If stored end_date is beyond one billing period from created_at (bad data), derive from created_at + period instead.
  ...
  ELSIF FOUND AND v_payment.created_at IS NOT NULL THEN
    -- Same logic as UI (useNextBillingFromPayments): next = created_at + 1 year/month
  ...
  -- Prefer organization_subscriptions.subscription_end_date when it matches overview/management (e.g. 376 days / 28 Mar 2027)
  -- so prorate "Remaining Subscription Days" matches the same source. Only use when it is in the future and within
  -- one billing period after the payment-derived end (avoids using wrong org data like 2028 when payment says 2027).
  ...
  -- Expose the end date we use for prorate in current_plan.end_date (for consistency)
...
'Prorate upgrade calculation. Uses subscription_end_date from latest successful payment when available so remaining_days matches UI (Renews / days remaining). Fallback: organization_subscriptions.subscription_end_date.';
