-- Fix prorate: use standard period length (365 yearly, 30 monthly) and correct member charge formula.
-- Logs showed total_days=29 for yearly -> wrong period; member_change_charge used 12/30 factor -> undercharge.
-- Correct: yearly = (base*12)*member_diff*(remaining_days/365), monthly = base*member_diff*(remaining_days/30).
...
BEGIN
  ...
  -- Use standard period length for prorate % and member charge: yearly=365, monthly=30.
  -- Fixes wrong total_days from (end-start) when data is monthly or inconsistent.
  ...
  -- Additional member charge: price per member for remaining period.
  -- Yearly: (base_per_member * 12 months) * member_diff * (remaining_days/365).
  -- Monthly: base_per_member * member_diff * (remaining_days/30).
  -- No 12/30 factor; use standard period length above.
...
'Prorate upgrade. Uses standard period length (365 days yearly, 30 monthly). Member charge: yearly = (base*12)*diff*(remaining/365), monthly = base*diff*(remaining/30).';
