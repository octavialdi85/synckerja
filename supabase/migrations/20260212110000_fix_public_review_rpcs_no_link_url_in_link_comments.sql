-- get_public_review_comments: filter by plan only (no link_url in link_comments), show commenter_display_name
...
-- insert_public_review_comment: no link_url (link_comments has no link_url column)
...
COMMENT ON FUNCTION public.get_public_review_comments(TEXT) IS 'Return all comments for token plan. No auth. Uses commenter_display_name when set.';
COMMENT ON FUNCTION public.insert_public_review_comment(TEXT, TEXT, TEXT, NUMERIC, JSONB) IS 'Insert a public comment with commenter display name (no auth).';
