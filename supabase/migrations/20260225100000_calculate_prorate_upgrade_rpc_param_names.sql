-- Fix RPC parameter names: Edge Function calls with org_id, new_member_count, target_plan_id.
-- PostgREST schema cache expects exact parameter names; previous migration used p_ prefix.
-- Drop and recreate with names matching the Edge Function invoke.
...
'Returns a single JSONB object for prorate upgrade calculation. Parameters: org_id, new_member_count, target_plan_id (matches Edge Function calculate-prorate).';
