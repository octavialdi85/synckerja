-- Notify both assignee and assigner when a step / sub-step is reopened (is_completed -> false).
-- Extends existing daily_task_notifications triggers without changing trigger names.

BEGIN;

-- Update Trigger 4 logic: task_steps UPDATE OF is_completed (reopened)
...
DECLARE
  ...
  v_rec RECORD;
BEGIN
  -- Only when step is reopened (NEW.is_completed = false and value changed)
  ...
  -- Notify both assignees and assigners (distinct user_ids)
  FOR v_rec IN
...
-- Update Trigger 5 logic: task_steps_to_steps UPDATE OF is_completed (reopened)
...
DECLARE
  ...
  v_rec RECORD;
BEGIN
  -- Only when sub-step is reopened (NEW.is_completed = false and value changed)
  IF NEW.is_completed = true THEN
    RETURN NEW;
  END IF;
  IF OLD.is_completed IS NOT DISTINCT FROM NEW.is_completed THEN
