-- Content Plan + Content Post: status change -> "Content Plan", done change -> "Content Post"
-- (same two-line body format as Content Production). Remove "Update status" / "Plan selesai".
...
COMMENT ON FUNCTION public.trigger_notify_plan_status_change() IS 'Notify org members when status, production_status, or done changes. production_status: title "Content Production", body "Old → New" + newline + plan title. status: title "Content Plan", same body format. done: title "Content Post", body "Not posted → Posted" or "Posted → Not posted" + newline + plan title. No "Update status" or "Plan selesai".';
