...
export const Filters = () => {
  ...
  return (
    <div className="p-2 md:p-2 bg-white border border-gray-200 rounded-md">
      <div className="flex flex-col md:flex-row flex-wrap gap-2 md:gap-1 items-stretch md:items-center">
        {/* Search */}
        <div className="relative flex-1 min-w-full md:min-w-[150px]">
          <Search className="w-4 h-4 text-gray-400 absolute left-3 top-1/2 transform -translate-y-1/2 z-10 pointer-events-none" />
          <Input
            type="text"
            value={filters.search}
            onChange={(e) => updateFilter('search', e.target.value)}
            placeholder="Search employee, task, step..."
            className="w-full pl-10 pr-4 h-9 md:h-9 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent text-sm"
          />
        </div>

        {/* Status */}
        <Select value={filters.status} onValueChange={(v) => updateFilter('status', v)}>
          <SelectTrigger className="w-full md:w-32 lg:w-36 h-9 text-sm text-gray-700 text-left">
        ...
        <Select value={filters.pic || 'all'} onValueChange={(v) => updateFilter('pic', v)}>
          <SelectTrigger className="w-full md:w-36 h-9 text-sm text-gray-700 text-left">
        ...
        <button
          onClick={handleClear}
          className="h-9 px-3 hover:bg-gray-100 rounded-md transition-colors border border-gray-300 flex items-center justify-center w-full md:w-auto"
          title="Clear filters"
        >
