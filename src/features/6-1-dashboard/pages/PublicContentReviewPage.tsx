  ...
  return (
    <div className="min-h-screen h-screen max-h-screen bg-gray-50 flex flex-col overflow-y-auto overflow-x-hidden overscroll-behavior-y-contain">
      <Dialog open={showCommenterPopup} onOpenChange={setShowCommenterPopup}>
        <DialogContent overlayClassName="bg-black/80" className="max-w-[360px] w-[calc(100%-2rem)] border-0 bg-[#1C1C28] shadow-xl rounded-xl p-6 text-left [&>button]:text-gray-300 [&>button]:hover:text-white">
          <DialogHeader>
          ...
          <div className="space-y-4 pt-1">
            <div className="space-y-2">
              <label htmlFor="commenter-name" className="text-sm font-medium text-gray-300">
                Your name
              </label>
              <Input
                id="commenter-name"
                ...
                className="w-full bg-[#282838] border-[#282838] text-gray-100 placeholder:text-gray-500 rounded-lg focus-visible:ring-2 focus-visible:ring-[#6D5DFF] focus-visible:border-[#6D5DFF]"
              />
            </div>
            <Button
              onClick={handleSaveCommenterName}
              disabled={!popupNameInput.trim()}
              className="w-full bg-[#6D5DFF] hover:bg-[#5b4de6] text-white rounded-lg font-medium"
            >
              Save
            </Button>
          </div>
        ...
        <div className="bg-white rounded-lg border border-gray-200 shadow-sm flex flex-col flex-1 min-h-0">
          ...
          <div className="flex-1 overflow-auto p-4 space-y-3 min-h-[200px]">
            {comments.length === 0 ? (
              ...
              comments.map((c) => (
                <div key={c.id} className="flex flex-col max-w-[85%]">
                  <div className="rounded-2xl rounded-tl-md bg-gray-100 border border-gray-200/80 shadow-sm px-4 py-2.5">
                    <div className="flex items-center gap-2 text-xs text-gray-600 mb-1.5">
                      <span className="font-medium text-gray-800">{c.creator_display_name}</span>
                      <span className="text-gray-500">{c.created_at ? format(new Date(c.created_at), 'dd MMM yyyy, HH:mm') : ''}</span>
                    </div>
