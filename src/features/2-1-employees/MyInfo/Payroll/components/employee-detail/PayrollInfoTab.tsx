...
export const PayrollInfoTab = ({ employee, isEditMode, onUpdate }: PayrollInfoTabProps) => {
  ...
  const { data: payrollInfo, isLoading, error } = useEmployeePayrollInfo(employeeId || '');
  const { data: payrollComponents = [] } = usePayrollComponents(payrollInfo?.id);
  const savePayrollMutation = useSavePayrollInfo(employeeId || '');
  const addComponentMutation = useAddPayrollComponent(employeeId || '');
  const updateComponentMutation = useUpdatePayrollComponent();
  const deleteComponentMutation = useDeletePayrollComponent();
  ...
  const handleSaveSection = async (section: string, e?: React.MouseEvent) => {
    if (e) {
      e.preventDefault();
      e.stopPropagation();
    }
    ...
    try {
      setIsSaving(true);
      await savePayrollMutation.mutateAsync(formData);
      toast.success('Payroll information saved successfully');
      setEditingSection(null); // Close edit mode after save
      // React Query auto-handles invalidation & refetch - no manual reload needed
    } catch (error) {
  ...
  const handleAddComponent = async () => {
    ...
    if (!payrollInfo?.id) {
      toast.error('Please save payroll information first');
      return;
    }

    try {
      await addComponentMutation.mutateAsync({ 
        component: newComponent as Omit<PayrollComponent, 'id' | 'employee_payroll_info_id' | 'organization_id'>,
  ...
  const handleUpdateComponent = async (id: string, updatedData: Partial<PayrollComponent>) => {
    try {
      await updateComponentMutation.mutateAsync({ id, updates: updatedData });
  ...
  const handleDeleteComponent = async (id: string) => {
    ...
    try {
      await deleteComponentMutation.mutateAsync({ id, payrollInfoId: payrollInfo.id });
      // Success toast already handled by mutation
    } catch (error: any) {
            ...
            <div className="flex items-center space-x-2">
              {editingSection === 'basic-salary' ? (
                <>
                  ...
                  <Button 
                    type="button"
                    onClick={(e) => handleSaveSection('basic-salary', e)} 
                    disabled={isSaving}
                ...
                <Button 
                  onClick={() => handleEditSection('basic-salary')}
                  variant="outline"
                  size="sm"
                >
            ...
            <div className="flex items-center space-x-2">
              {editingSection === 'tax-employment' ? (
                <>
                  ...
                  <Button 
                    type="button"
                    onClick={() => handleSaveSection('tax-employment')} 
                    disabled={isSaving}
                    className="bg-green-600 hover:bg-green-700"
                    size="sm"
                  >
                ...
                <Button 
                  onClick={() => handleEditSection('tax-employment')}
                  variant="outline"
                  size="sm"
                >
                  <Edit className="h-4 w-4 mr-2" />
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            <div className="space-y-2">
              ...
              <Select
                ...
                disabled={editingSection !== 'tax-employment' || isSaving}
              >
                <SelectTrigger>
                  <SelectValue />
                </SelectTrigger>
            ...
            <div className="flex items-center space-x-2">
              {editingSection === 'bpjs' ? (
                <>
                  ...
                  <Button 
                    type="button"
                    onClick={() => handleSaveSection('bpjs')} 
                    disabled={isSaving}
                    className="bg-green-600 hover:bg-green-700"
                    size="sm"
                  >
                ...
                <Button 
                  onClick={() => handleEditSection('bpjs')}
                  variant="outline"
                  size="sm"
                >
                  <Edit className="h-4 w-4 mr-2" />
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            <div className="space-y-2">
              ...
              <Input
                ...
                disabled={editingSection !== 'bpjs' || isSaving}
              />
            </div>
            <div className="space-y-2">
            ...
            <div className="flex items-center space-x-2">
              {editingSection === 'banking' ? (
                <>
                  ...
                  <Button 
                    type="button"
                    onClick={() => handleSaveSection('banking')} 
                    disabled={isSaving}
                    className="bg-green-600 hover:bg-green-700"
                    size="sm"
                  >
