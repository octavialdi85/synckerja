...
const EmployeeAttendance = () => {
  ...
  const handleSaveChanges = async () => {
    // Trigger save for attendance if available
    if (typeof window !== 'undefined' && (window as any).saveAttendance) {
      await (window as any).saveAttendance();
    }
    setIsEditMode(false);
  };
                      ...
                      <div className="bg-white rounded-lg border border-gray-200 shadow-sm flex flex-col h-full">
                        <div className="flex-1 overflow-y-auto seamless-scroll min-h-0">
                          <div className="p-6 h-full">
                            ...
                            <PersonalAttendanceCalendar employeeId={employeeId} />
                          </div>
                        </div>

                        {/* Content Footer */}
                        <MyInfoContentFooter section="Attendance" />
                      </div>
                    </div>
                  </div>
                </div>
