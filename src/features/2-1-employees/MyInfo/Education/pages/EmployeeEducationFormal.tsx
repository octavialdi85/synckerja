...
import { EducationInfoTab } from '../components/employee-detail/EducationInfoTab';
import { useEmployeeDetail } from '../../Education/hooks/useEmployeeDetail';
import { useAvatarSync } from '../../Education/hooks/useAvatarSync';
import { toast } from 'sonner';
import { EducationTableFooter } from '../components/EducationTableFooter';
import { EducationSidebarFooter } from '../components/EducationSidebarFooter';

const EmployeeEducationFormal = () => {
  const navigate = useNavigate();
  const { id } = useParams();
  const [searchParams] = useSearchParams();
  const employeeId = id || searchParams.get('id');
  const isEditMode = true; // Always allow editing education records
  ...
  // Calculate total education records (this should come from actual data)
  const totalEducation = 0; // This should be calculated from actual education data
  
  // Get highest degree (placeholder)
  const highestDegree = 'Bachelor'; // This should come from actual education data
...
export default EmployeeEducationFormal;
