...
import { AddressInfoTab } from '../components/employee-detail/AddressInfoTab';
import { useEmployeeDetail } from '../../AddressInformation/hooks/useEmployeeDetail';
import { useAvatarSync } from '../../AddressInformation/hooks/useAvatarSync';
import { toast } from 'sonner';
import { MyInfoSidebarFooter } from '../../shared/MyInfoSidebarFooter';
import { MyInfoContentFooter } from '../../shared/MyInfoContentFooter';
...
const EmployeeAddressInfo = () => {
  ...
  const handleSaveChanges = async () => {
    if (typeof window !== 'undefined' && (window as any).addressTabSave) {
      const success = await (window as any).addressTabSave();
      if (success) {
        setIsEditMode(false);
      }
    }
                    ...
                    <div className="bg-white rounded-lg border border-gray-200 shadow-sm flex flex-col h-full">
                      <div className="flex-1 overflow-y-auto seamless-scroll min-h-0">
                        <div className="p-6">
                          ...
                          <AddressInfoTab employee={employee} isEditMode={isEditMode} onUpdate={() => {}} />
                        </div>
                      </div>

                      {/* Content Footer */}
                      <MyInfoContentFooter section="Address Information" />
                    </div>
                  </div>
                </div>
              </div>
