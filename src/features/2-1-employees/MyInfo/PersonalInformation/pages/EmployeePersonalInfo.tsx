...
import { PersonalInfoTab } from '../components/employee-detail/PersonalInfoTab';
...
const EmployeePersonalInfo = () => {
  ...
  const handleSaveChanges = async () => {
    if (typeof window !== 'undefined' && (window as any).savePersonalInfo) {
      const success = await (window as any).savePersonalInfo();
      if (success) {
        setIsEditMode(false);
      }
    }
                  ...
                  <div className="col-span-9 flex flex-col min-h-0">
                    ...
                    <div className="flex-1 min-h-0">
                      <div className="h-full bg-white rounded-lg border border-gray-200 shadow-sm flex flex-col max-h-[calc(100vh-135px)]">
                        ...
                        {/* Content Footer */}
                        <MyInfoContentFooter section="Personal Information" />
                      </div>
                    </div>
                  </div>
                </div>
