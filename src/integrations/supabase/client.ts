...
function getSupabaseClient(): ReturnType<typeof createClient<Database>> {
  ...
  global: {
    ...
    fetch: async (url: RequestInfo | URL, options: RequestInit = {}) => {
      ...
      // Edge Functions (e.g. generate-design-image): long timeout (5 min) so AI image generation with reference images can finish
      const isLoginRequest = isAuthRequest && !isRefreshToken;
      const MAX_RETRIES = isRefreshToken ? 1 : (isAuthRequest ? 1 : 1);
      const TIMEOUT_MS = isRefreshToken ? 18000 : (isEdgeFunction ? 300000 : (isAuthRequest ? 40000 : 25000));
