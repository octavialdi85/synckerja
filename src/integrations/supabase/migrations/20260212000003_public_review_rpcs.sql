-- RPC 1: Resolve token and return content data for public review (no auth)
...
-- RPC 2: Get all comments (internal + public) for the token's content (no auth)
  ...
  SELECT COALESCE(json_agg(
    json_build_object(
      ...
      'creator_display_name', CASE
        WHEN lc.created_by IS NULL THEN 'Anonim'
        ELSE COALESCE((SELECT p.full_name FROM public.profiles p WHERE p.user_id = lc.created_by LIMIT 1), 'Unknown')
      END
...
-- RPC 3: Insert public comment (no auth; created_by NULL)
...
BEGIN
  ...
  RETURN json_build_object(
    ...
    'creator_display_name', 'Anonim'
  );
END;
$$;

COMMENT ON FUNCTION public.get_public_review_content_by_token(TEXT) IS 'Resolve public review token and return content metadata for preview. No auth.';
COMMENT ON FUNCTION public.get_public_review_comments(TEXT) IS 'Return all comments (internal + public) for token content. No auth.';
COMMENT ON FUNCTION public.insert_public_review_comment(TEXT, TEXT, NUMERIC, JSONB) IS 'Insert a public (anonymous) comment for token content. No auth.';
